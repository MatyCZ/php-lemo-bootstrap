...
class FormLabel extends AbstractHelper
{
    ...
    public function openTag($attributesOrElement = null)
    {
        if(null === $attributesOrElement) {
            $attributesOrElement['class'] = 'control-label';
        } else {
            if(is_array($attributesOrElement)) {
                if(array_key_exists('class', $attributesOrElement)) {
                    if(false === strpos($attributesOrElement['class'], 'control-label')) {
                        $attributesOrElement['class'] = trim($attributesOrElement['class'] . ' ' . 'control-label');
                    }
                } else {
                    $attributesOrElement['class'] = 'control-label';
                }
            ...
            if($attributesOrElement instanceof ElementInterface) {
                if($attributesOrElement->hasAttribute('class')) {
                    if(false === strpos($attributesOrElement->getAttribute('class'), 'control-label')) {
                        $attributesOrElement->setAttribute('class', trim($attributesOrElement->getAttribute('class') . ' ' . 'control-label'));
                    }
                } else {
                    $attributesOrElement->setAttribute('class', 'control-label');
                }
            }
        }
